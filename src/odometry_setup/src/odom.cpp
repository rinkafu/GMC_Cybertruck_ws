...
double DistancePerCount = (3.14159265 * 0.15) / 16000;
...
int main(int argc, char** argv){
 ...
 current_time = ros::Time::now();
  ...
  while(n.ok()){
	...
	double mt_l = encLeft-encLeft_old;
	double mt_r = encRight-encRight_old;
	if(mt_l>=1000000)mt_l-=1000000;
	if(mt_l<=-100000)mt_l+=1000000;
	if(mt_r>=1000000)mt_r-=1000000;
        if(mt_r<=-100000)mt_r+=1000000;
		dist_left = mt_l * DistancePerCount;
  		dist_right = mt_r * DistancePerCount;
	double dist =  (dist_right + dist_left) * 0.5;
    	//double rota = (dist_left - dist_right) / 0.51; //Distance L-R
	double right_left = dist_right - dist_left;
	double wheelbase = 0.51;
	double rota = right_left / wheelbase;
